...
# Warning: Do not supported Windows on Arm
...
def _rotl8(Value, Shift):
    return UCHAR(Value).value, UCHAR(Shift).value

def _rotl16(Value, Shift):
    return USHORT(Value).value, UCHAR(Shift).value

def _rotr8(Value, Shift):
    return UCHAR(Value).value, UCHAR(Shift).value

def _rotr16(Value, Shift):
    return USHORT(Value).value, UCHAR(Shift).value
...
def _rotl(Value, Shift):
    return UINT(Value).value, INT(Shift).value

def _rotr(Value, Shift):
    return UINT(Value).value, INT(Shift).value

def _rotl64(Value, Shift):
    return UINT64(Value).value, INT(Shift).value

def _rotr64(Value, Shift):
    return UINT64(Value).value, INT(Shift).value
...
def RTL_FIELD_SIZE(type, field):
    class TempStruct(type):
        _fields_ = type._fields_
    
    field_type = None
    for name, ftype in TempStruct._fields_:
        if name == field:
            field_type = ftype
            break
    
    if field_type is None:
        raise AttributeError(f"Field '{field}' not found in struct")
    
    if hasattr(type, '_length_') and hasattr(field, '_type_'):
        element_size = sizeof(field_type._type_)
        return element_size * field_type._length_
...
DBG_EXCEPTION_HANDLED = DWORD(0x00010001).value
DBG_CONTINUE = DWORD(0x00010002).value
STATUS_SEGMENT_NOTIFICATION = DWORD(0x40000005).value
STATUS_FATAL_APP_EXIT = DWORD(0x40000015).value
DBG_REPLY_LATER = DWORD(0x40010001).value
DBG_TERMINATE_THREAD = DWORD(0x40010003).value
DBG_TERMINATE_PROCESS = DWORD(0x40010004).value
DBG_CONTROL_C = DWORD(0x40010005).value
DBG_PRINTEXCEPTION_C = DWORD(0x40010006).value
DBG_RIPEXCEPTION = DWORD(0x40010007).value
DBG_CONTROL_BREAK = DWORD(0x40010008).value
DBG_COMMAND_EXCEPTION = DWORD(0x40010009).value
DBG_PRINTEXCEPTION_WIDE_C = DWORD(0x4001000A).value
...
DBG_EXCEPTION_NOT_HANDLED = DWORD(0x80010001).value
...
def IS_DISPATCHING(f):
    return (f & EXCEPTION_UNWIND) == 0


def IS_TARGET_UNWIND(f):
...
REPARSE_GUID_DATA_BUFFER_HEADER_SIZE = FIELD_OFFSET(REPARSE_GUID_DATA_BUFFER, REPARSE_GUID_DATA_BUFFER.GenericReparseBuffer)
...
def IMAGE_FIRST_SECTION(ntheader):
    pass
    # ((PIMAGE_SECTION_HEADER) ((ULONG_PTR)ntheader + FIELD_OFFSET(IMAGE_NT_HEADERS,OptionalHeader) + ((PIMAGE_NT_HEADERS)(ntheader))->FileHeader.SizeOfOptionalHeader))
...
RtlAddFunctionTable = NTAPI(BOOLEAN, PRUNTIME_FUNCTION, DWORD, DWORD64)
RtlDeleteFunctionTable = NTAPI(BOOLEAN, PRUNTIME_FUNCTION)
RtlInstallFunctionTableCallback = NTAPI(BOOLEAN, DWORD64, DWORD64, DWORD, PGET_RUNTIME_FUNCTION_CALLBACK, PVOID, PCWSTR)
RtlRestoreContext = NTAPI(VOID, PCONTEXT, _EXCEPTION_RECORD)

RtlUnwind = NTAPI(VOID, PVOID, PVOID, PEXCEPTION_RECORD, PVOID)
RtlPcToFileHeader = NTAPI(VOID, PVOID, PVOID)

# _x86_64

RtlLookupFunctionEntry = NTAPI(PRUNTIME_FUNCTION, DWORD64, PDWORD64, PUNWIND_HISTORY_TABLE)
RtlUnwindEx = NTAPI(VOID, PVOID, PVOID, PEXCEPTION_RECORD, PVOID, PCONTEXT, PUNWIND_HISTORY_TABLE)
RtlVirtualUnwind = NTAPI(PEXCEPTION_ROUTINE, 
                         DWORD, 
                         DWORD64, 
                         DWORD64, 
                         PRUNTIME_FUNCTION, 
...
RtlInitializeSListHead = NTAPI(VOID, PSLIST_HEADER)
RtlFirstEntrySList = NTAPI(PSLIST_ENTRY, SLIST_HEADER)
RtlInterlockedPopEntrySList = NTAPI(PSLIST_ENTRY, PSLIST_HEADER)
RtlInterlockedPushEntrySList = NTAPI(PSLIST_ENTRY, PSLIST_HEADER, PSLIST_ENTRY)
RtlInterlockedPushListSListEx = NTAPI(PSLIST_ENTRY, PSLIST_HEADER, PSLIST_ENTRY, PSLIST_ENTRY, DWORD)
RtlInterlockedFlushSList = NTAPI(PSLIST_ENTRY, PSLIST_HEADER)
RtlQueryDepthSList = NTAPI(WORD, PSLIST_HEADER)
...
PVECTORED_EXCEPTION_HANDLER = NTAPI(LONG, _EXCEPTION_POINTERS)
