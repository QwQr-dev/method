...
PIMAGE_TLS_CALLBACK = CALLBACK(VOID, PVOID, DWORD, PVOID)
...
def VerSetConditionMask(ConditionMask, TypeMask, Condition) -> int:
...
def RtlGetProductInfo(OSMajorVersion, OSMinorVersion, SpMajorVersion, SpMinorVersion, ReturnedProductType) -> int:
...
RTL_UMS_SCHEDULER_ENTRY_POINT = CALLBACK(VOID, UINT, ULONG_PTR, PVOID)
...
if WIN32_WINNT >= 0x0602:
    ...
    def RtlCrc32(Buffer, Size, InitialCrc) -> int:
        RtlCrc32 = ntdll.RtlCrc32
        RtlCrc32.argtypes = [PVOID, c_size_t, DWORD]
        RtlCrc32.restype = DWORD
        res = RtlCrc32(Buffer, Size, InitialCrc)
        return res


    def RtlCrc64(Buffer, Size, InitialCrc) -> int:
        RtlCrc64 = ntdll.RtlCrc64
        RtlCrc64.argtypes = [PVOID, c_size_t, DWORD]
        RtlCrc64.restype = DWORD
...
PAPCFUNC = POINTER(CALLBACK(VOID, ULONG_PTR))
PVECTORED_EXCEPTION_HANDLER = POINTER(CALLBACK(LONG, _EXCEPTION_POINTERS))
...
WORKERCALLBACKFUNC = CALLBACK(VOID, PVOID)
APC_CALLBACK_FUNCTION = CALLBACK(VOID, DWORD, PVOID, PVOID)
WAITORTIMERCALLBACKFUNC = CALLBACK(VOID, PVOID, BOOLEAN)
WAITORTIMERCALLBACK = WAITORTIMERCALLBACKFUNC
PFLS_CALLBACK_FUNCTION = CALLBACK(VOID, PVOID)
PSECURE_MEMORY_CACHE_CALLBACK = CALLBACK(BOOLEAN, PVOID, SIZE_T)
...
RTL_VERIFIER_DLL_LOAD_CALLBACK = CALLBACK(VOID, PWSTR, PVOID, SIZE_T, PVOID)
RTL_VERIFIER_DLL_UNLOAD_CALLBACK = CALLBACK(VOID, PWSTR, PVOID, SIZE_T, PVOID)
RTL_VERIFIER_NTDLLHEAPFREE_CALLBACK = CALLBACK(VOID, PVOID, SIZE_T)
...
def RtlApplicationVerifierStop(
    Code, 
    Message, 
    Param1, 
    Description1, 
    ...
    RtlApplicationVerifierStop = ntdll.RtlApplicationVerifierStop
    RtlApplicationVerifierStop.argtypes = [ULONG_PTR, PSTR, ULONG_PTR, PSTR, ULONG_PTR, PSTR, ULONG_PTR, PSTR, ULONG_PTR, PSTR]
    RtlApplicationVerifierStop.restype = DWORD
    res = RtlApplicationVerifierStop(Code, Message, Param1, Description1, Param2, Description2, Param3, Description3,Param4,Description4)
    return res


def RtlSetHeapInformation(HeapHandle, HeapInformationClass, HeapInformation, HeapInformationLength) -> int:
    RtlSetHeapInformation = ntdll.RtlSetHeapInformation
    RtlSetHeapInformation.argtypes = [PVOID, UINT, PVOID, SIZE_T]
    RtlSetHeapInformation.restype = DWORD
    res = RtlSetHeapInformation(HeapHandle, HeapInformationClass, HeapInformation, HeapInformationLength)
    return res


def RtlQueryHeapInformation(HeapHandle, HeapInformationClass, HeapInformation, HeapInformationLength, ReturnLength) -> int:
    RtlQueryHeapInformation = ntdll.RtlQueryHeapInformation
    RtlQueryHeapInformation.argtypes = [PVOID, UINT, PVOID, SIZE_T, PSIZE_T]
    RtlQueryHeapInformation.restype = DWORD
    res = RtlQueryHeapInformation(HeapHandle, HeapInformationClass, HeapInformation, HeapInformationLength, ReturnLength)
    return res


def RtlMultipleAllocateHeap(HeapHandle, Flags, Size, Count, Array) -> int:
    RtlMultipleAllocateHeap = ntdll.RtlMultipleAllocateHeap
    RtlMultipleAllocateHeap.argtypes = [PVOID, DWORD, SIZE_T, DWORD, PVOID]
    RtlMultipleAllocateHeap.restype = DWORD
    res = RtlMultipleAllocateHeap(HeapHandle, Flags, Size, Count, Array)
    return res


def RtlMultipleFreeHeap(HeapHandle, Flags, Count, Array) -> int:
    RtlMultipleFreeHeap = ntdll.RtlMultipleFreeHeap
    RtlMultipleFreeHeap.argtypes = [PVOID, DWORD, DWORD, PVOID]
    RtlMultipleFreeHeap.restype = DWORD
    res = RtlMultipleFreeHeap(HeapHandle, Flags, Count, Array)
    return res


def VERIFIER_STOP(Code,Msg,P1,S1,P2,S2,P3,S3,P4,S4):
    return RtlApplicationVerifierStop(Code, Msg, ULONG_PTR(P1).value, S1, ULONG_PTR(P2).value, S2,ULONG_PTR(P3).value, S3, ULONG_PTR(P4).value, S4)
...
PTP_SIMPLE_CALLBACK = CALLBACK(VOID, PTP_CALLBACK_INSTANCE, PVOID)
...
PTP_CLEANUP_GROUP_CANCEL_CALLBACK = CALLBACK(VOID, PVOID, PVOID)
...
PTP_WORK_CALLBACK = CALLBACK(VOID, PTP_CALLBACK_INSTANCE, PVOID, PTP_WORK)
...
PTP_TIMER_CALLBACK = CALLBACK(VOID, PTP_CALLBACK_INSTANCE, PVOID, PTP_TIMER)
...
PTP_WAIT_CALLBACK = CALLBACK(VOID, PTP_CALLBACK_INSTANCE, PVOID, PTP_WAIT, TP_WAIT_RESULT)
