...
from method.System.wingdi import LF_FACESIZE
...
def SHGetMalloc(ppMalloc: Any, errcheck: bool = True):
...
def ILSaveToStream(pstm, pidl, errcheck: bool = True):
    ILSaveToStream = shell32.ILSaveToStream
    res = ILSaveToStream(pstm, pidl)
    return hresult_to_errcheck(res, errcheck)


def ILLoadFromStream(pstm, pidl, errcheck: bool = True):
...
if NTDDI_VERSION >= 0x06000000:
    def ILLoadFromStreamEx(pstm, pidl, errcheck: bool = True):
...
if NTDDI_VERSION >= 0x06000000:
    ...
    def SHGetPathFromIDListEx(pidl, pszPath, cchPath, uOpts, errcheck: bool = True):
...
def SHCreateDirectory(hwnd, pszPath, errcheck: bool = True):
    SHCreateDirectory = shell32.SHCreateDirectory
    res = SHCreateDirectory(hwnd, pszPath)
    return hresult_to_errcheck(res, errcheck)


def SHCreateDirectoryEx(hwnd, pszPath, psa, unicode: bool = True, errcheck: bool = True):
...
def SHCreateShellItem(pidlParent, psfParent, pidl, ppsi, errcheck: bool = True):
...
def SHGetSpecialFolderLocation(hwnd, csidl, ppidl, errcheck: bool = True):
...
def SHGetSpecialFolderPath(hwnd, pszPath, csidl, fCreate, unicode: bool = True, errcheck: bool = True):
...
def SHSetFolderPath(csidl, hToken, dwFlags, pszPath, unicode: bool = True, errcheck: bool = True):
...
def SHGetFolderLocation(hwnd, csidl, hToken, dwFlags, ppidl, errcheck: bool = True):
    SHGetFolderLocation = shell32.SHGetFolderLocation
    res = SHGetFolderLocation(hwnd, csidl, hToken, dwFlags, ppidl)
    return hresult_to_errcheck(res, errcheck)


def SHGetFolderPathAndSubDir(hwnd, csidl, hToken, dwFlags, pszSubDir, pszPath, unicode: bool = True, errcheck: bool = True):
...
if NTDDI_VERSION >= 0x06000000:
    ...
    def SHGetKnownFolderIDList(rfid, dwFlags, hToken, ppidl, errcheck: bool = True):
        SHGetKnownFolderIDList = shell32.SHGetKnownFolderIDList
        res = SHGetKnownFolderIDList(rfid, dwFlags, hToken, ppidl)
        return hresult_to_errcheck(res, errcheck)


    def SHSetKnownFolderPath(rfid, dwFlags, hToken, pszPath, errcheck: bool = True):
        SHSetKnownFolderPath = shell32.SHSetKnownFolderPath
        res = SHSetKnownFolderPath(rfid, dwFlags, hToken, pszPath)
        return hresult_to_errcheck(res, errcheck)
        

    def SHGetKnownFolderPath(rfid: Any, dwFlags: int, hToken: int, ppszPath: Any, errcheck: bool = True):
...
if NTDDI_VERSION >= 0x06010000:
    def SHGetKnownFolderItem(rfid, flags, hToken, riid, ppv, errcheck: bool = True):
