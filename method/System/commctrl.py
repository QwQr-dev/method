...
from method.System.guiddef import GUID, IID
from method.System.public_dll import comctl32
...
def ImageList_Copy(himlDst, iDst, himlSrc, iSrc, uFlags, errcheck: bool = True):
    ImageList_Copy = comctl32.ImageList_Copy
    res = ImageList_Copy(himlDst, iDst, himlSrc, iSrc, uFlags)
    return win32_to_errcheck(res, errcheck)


def ImageList_BeginDrag(himlTrack, iTrack, dxHotspot, dyHotspot, errcheck: bool = True):
...
def ImageList_DragEnter(hwndLock, x, y, errcheck: bool = True):
    ImageList_DragEnter = comctl32.ImageList_DragEnter
    res = ImageList_DragEnter(hwndLock, x, y)
    return win32_to_errcheck(res, errcheck)


def ImageList_DragLeave(hwndLock, errcheck: bool = True):
    ImageList_DragLeave = comctl32.ImageList_DragLeave
    res = ImageList_DragLeave(hwndLock)
    return win32_to_errcheck(res, errcheck)


def ImageList_DragMove(x, y, errcheck: bool = True):
    ImageList_DragMove = comctl32.ImageList_DragMove
    res = ImageList_DragMove(x, y)
    return win32_to_errcheck(res, errcheck)


def ImageList_SetDragCursorImage(himlDrag, iDrag, dxHotspot, dyHotspot, errcheck: bool = True):
...
def ImageList_DragShowNolock(fShow, errcheck: bool = True):
    ImageList_DragShowNolock = comctl32.ImageList_DragShowNolock
    res = ImageList_DragShowNolock(fShow)
    return win32_to_errcheck(res, errcheck)


def ImageList_GetDragImage(ppt, pptHotspot, errcheck: bool = True):
...
def ImageList_Read(pstm, errcheck: bool = True):
    ImageList_Read = comctl32.ImageList_Read
    res = ImageList_Read(pstm)
    return win32_to_errcheck(res, errcheck)


def ImageList_Write(himl, pstm, errcheck: bool = True):
...
def ImageList_ReadEx(dwFlags, pstm, riid, ppv, errcheck: bool = True):
    ImageList_ReadEx = comctl32.ImageList_ReadEx
    res = ImageList_ReadEx(dwFlags, pstm, riid, ppv)
    return win32_to_errcheck(res, errcheck)


def ImageList_WriteEx(himl, dwFlags, pstm, errcheck: bool = True):
...
def CreateMappedBitmap(hInstance, idBitmap, wFlags, lpColorMap, iNumMaps, errcheck: bool = True):
...
def CreateStatusWindow(style, lpszText, hwndParent, wID, unicode: bool = True, errcheck: bool = True):
...
def ShowHideMenuCtl(hwnd, uFlags, lpInfo, errcheck: bool = True):
...
def CreateUpDownControl(dwStyle, x, y, cx, cy, hParent, nID, hInst, hBuddy, nUpper, nLower, nPos, errcheck: bool = True):
...
LPSTR_TEXTCALLBACKW = LPVOID(INT_PTR(-1).value).value  
LPSTR_TEXTCALLBACKA = LPVOID(INT_PTR(-1).value).value   
